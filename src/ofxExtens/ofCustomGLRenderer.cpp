...
const string ofCustomGLRenderer::TYPE="ProgrammableGL";
...
//----------------------------------------------------------
ofCustomGLRenderer::ofCustomGLRenderer(const ofAppBaseWindow * _window)
:ofGLProgrammableRenderer(_window),matrixStack(_window)
,graphics3d(this)
{
...
//----------------------------------------------------------
void ofCustomGLRenderer::startRender() {
...
//----------------------------------------------------------
void ofCustomGLRenderer::finishRender() {
	if (!uniqueShader) {
		glUseProgram(0);
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofMesh & vertexData, ofPolyRenderMode renderType, bool useColors, bool useTextures, bool useNormals) const{
	...
	const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,useColors,useTextures,useNormals);
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofVboMesh & mesh, ofPolyRenderMode renderType) const{
	drawInstanced(mesh,renderType,1);
}

//----------------------------------------------------------
void ofCustomGLRenderer::drawInstanced(const ofVboMesh & mesh, ofPolyRenderMode renderType, int primCount) const{
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw( const of3dPrimitive& model, ofPolyRenderMode renderType) const {
	const_cast<ofCustomGLRenderer*>(this)->pushMatrix();
	const_cast<ofCustomGLRenderer*>(this)->multMatrix(model.getGlobalTransformMatrix());
	if(model.isUsingVbo()){
		draw(static_cast<const ofVboMesh&>(model.getMesh()),renderType);
	}else{
		draw(model.getMesh(),renderType);
	}
	const_cast<ofCustomGLRenderer*>(this)->popMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofNode& node) const{
	const_cast<ofCustomGLRenderer*>(this)->pushMatrix();
	const_cast<ofCustomGLRenderer*>(this)->multMatrix(node.getGlobalTransformMatrix());
	node.customDraw(this);
	const_cast<ofCustomGLRenderer*>(this)->popMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofPolyline & poly) const{
	...
	const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,false,false,false);
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofPath & shape) const{
	ofColor prevColor;
	if(shape.getUseShapeColor()){
		prevColor = currentStyle.color;
	}
	ofCustomGLRenderer * mut_this = const_cast<ofCustomGLRenderer*>(this);
	if(shape.isFilled()){
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const{
	if(image.isUsingTexture()){
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,false,true,false);
		const ofTexture& tex = image.getTexture();
		if(tex.isAllocated()) {
			const_cast<ofCustomGLRenderer*>(this)->bind(tex,0);
			draw(tex.getMeshForSubsection(x,y,z,w,h,sx,sy,sw,sh,isVFlipped(),currentStyle.rectMode),OF_MESH_FILL,false,true,false);
			const_cast<ofCustomGLRenderer*>(this)->unbind(tex,0);
		} else {
			ofLogWarning("ofCustomGLRenderer") << "draw(): texture is not allocated";
		}
	}
}

//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofFloatImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const{
	if(image.isUsingTexture()){
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,false,true,false);
		const ofTexture& tex = image.getTexture();
		if(tex.isAllocated()) {
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofShortImage & image, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const{
	if(image.isUsingTexture()){
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,false,true,false);
		const ofTexture& tex = image.getTexture();
		if(tex.isAllocated()) {
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofTexture & tex, float x, float y, float z, float w, float h, float sx, float sy, float sw, float sh) const{
	const_cast<ofCustomGLRenderer*>(this)->setAttributes(true,false,true,false);
	if(tex.isAllocated()) {
		const_cast<ofCustomGLRenderer*>(this)->bind(tex,0);
		draw(tex.getMeshForSubsection(x,y,z,w,h,sx,sy,sw,sh,isVFlipped(),currentStyle.rectMode),OF_MESH_FILL,false,true,false);
...
//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofBaseVideoDraws & video, float x, float y, float w, float h) const{
	if(!video.isInitialized() || !video.isUsingTexture() || video.getTexturePlanes().empty()){
		return;
	}
	const_cast<ofCustomGLRenderer*>(this)->bind(video);
	...
	const_cast<ofCustomGLRenderer*>(this)->unbind(video);
}

//----------------------------------------------------------
void ofCustomGLRenderer::draw(const ofVbo & vbo, GLuint drawMode, int first, int total) const{
	if(vbo.getUsingVerts()) {
		//vbo.bind();
		GLuint VAO = vbo.getVaoId();
		glBindVertexArray(VAO);
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(vbo.getUsingVerts(),vbo.getUsingColors(),vbo.getUsingTexCoords(),vbo.getUsingNormals());
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawElements(const ofVbo & vbo, GLuint drawMode, int amt, int offsetelements) const{
	if(vbo.getUsingVerts()) {
		GLuint vaoID = vbo.getVaoId();
		if(vaoID!=0) {
			//vbo.bind();
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawInstanced(const ofVbo & vbo, GLuint drawMode, int first, int total, int primCount) const{
	if(vbo.getUsingVerts()) {
		vbo.bind();
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(vbo.getUsingVerts(),vbo.getUsingColors(),vbo.getUsingTexCoords(),vbo.getUsingNormals());
#ifdef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawElementsInstanced(const ofVbo & vbo, GLuint drawMode, int amt, int primCount) const{
	if(vbo.getUsingVerts()) {
		vbo.bind();
		const_cast<ofCustomGLRenderer*>(this)->setAttributes(vbo.getUsingVerts(),vbo.getUsingColors(),vbo.getUsingTexCoords(),vbo.getUsingNormals());
#ifdef TARGET_OPENGLES
...
//----------------------------------------------------------
ofPath & ofCustomGLRenderer::getPath(){
	return path;
}

//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofBaseVideoDraws & video){
	if(!video.isInitialized() || !video.isUsingTexture() || video.getTexturePlanes().empty()){
		return;
	}
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofBaseVideoDraws & video){
	if(!video.isInitialized() || !video.isUsingTexture() || video.getTexturePlanes().empty()){
		return;
	}
	if(usingVideoShader){
...
//----------------------------------------------------------
void ofCustomGLRenderer::pushView() {
	matrixStack.pushView();
}

//----------------------------------------------------------
void ofCustomGLRenderer::popView() {
	matrixStack.popView();
	uploadMatrices();
...
//----------------------------------------------------------
void ofCustomGLRenderer::viewport(ofRectangle viewport_){
	viewport(viewport_.x,viewport_.y,viewport_.width,viewport_.height,isVFlipped());
}

//----------------------------------------------------------
void ofCustomGLRenderer::viewport(float x, float y, float width, float height, bool vflip) {
...
//----------------------------------------------------------
ofRectangle ofCustomGLRenderer::getCurrentViewport() const{
	return matrixStack.getCurrentViewport();
}

//----------------------------------------------------------
ofRectangle ofCustomGLRenderer::getNativeViewport() const{
    return matrixStack.getNativeViewport();
}

//----------------------------------------------------------
int ofCustomGLRenderer::getViewportWidth() const{
	return getCurrentViewport().width;
}

//----------------------------------------------------------
int ofCustomGLRenderer::getViewportHeight() const{
	return getCurrentViewport().height;
}

//----------------------------------------------------------
bool ofCustomGLRenderer::isVFlipped() const{
	return matrixStack.isVFlipped();
}

//----------------------------------------------------------
void ofCustomGLRenderer::setCoordHandedness(ofHandednessType handedness) {
	;
}

//----------------------------------------------------------
ofHandednessType ofCustomGLRenderer::getCoordHandedness() const{
	return matrixStack.getHandedness();
}

//----------------------------------------------------------
void ofCustomGLRenderer::setOrientation(ofOrientation orientation, bool vFlip){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setupScreenPerspective(float width, float height, float fov, float nearDist, float farDist) {
	float viewW, viewH;
	if(width<0 || height<0){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setupScreenOrtho(float width, float height, float nearDist, float farDist) {
	float viewW, viewH;
	if(width<0 || height<0){
		ofRectangle currentViewport = getCurrentViewport();
...
void ofCustomGLRenderer::setupGraphicDefaults(){
	setStyle(ofStyle());
	path.setMode(ofPath::POLYLINES);
	path.setUseShapeColor(false);
}
...
//----------------------------------------------------------
void ofCustomGLRenderer::setupScreen(){
	beginDefaultShader();
	setupScreenPerspective();	// assume defaults
}

//----------------------------------------------------------
void ofCustomGLRenderer::setCircleResolution(int res){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setPolyMode(ofPolyWindingMode mode){
...
//our openGL wrappers
//----------------------------------------------------------
void ofCustomGLRenderer::pushMatrix(){
	matrixStack.pushMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::popMatrix(){
	matrixStack.popMatrix();
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::translate(const glm::vec3& p){
	translate(p.x, p.y, p.z);
}

//----------------------------------------------------------
void ofCustomGLRenderer::translate(float x, float y, float z){
	matrixStack.translate(x,y,z);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::scale(float xAmnt, float yAmnt, float zAmnt){
	matrixStack.scale(xAmnt, yAmnt, zAmnt);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::rotateRad(float radians, float vecX, float vecY, float vecZ){
	matrixStack.rotateRad(radians, vecX, vecY, vecZ);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::rotateXRad(float radians){
	rotateRad(radians, 1, 0, 0);
}

//----------------------------------------------------------
void ofCustomGLRenderer::rotateYRad(float radians){
	rotateRad(radians, 0, 1, 0);
}

//----------------------------------------------------------
void ofCustomGLRenderer::rotateZRad(float radians){
	rotateRad(radians, 0, 0, 1);
}

//same as ofRotateZ
//----------------------------------------------------------
void ofCustomGLRenderer::rotateRad(float radians){
	rotateZRad(radians);
}

//----------------------------------------------------------
void ofCustomGLRenderer::matrixMode(ofMatrixMode mode){
	matrixStack.matrixMode(mode);
}

//----------------------------------------------------------
void ofCustomGLRenderer::loadIdentityMatrix (void){
	matrixStack.loadIdentityMatrix();
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::loadMatrix (const glm::mat4 & m){
	matrixStack.loadMatrix(m);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::loadMatrix (const float *m){
	loadMatrix(glm::make_mat4(m));
}

//----------------------------------------------------------
void ofCustomGLRenderer::multMatrix (const glm::mat4 & m){
	matrixStack.multMatrix(m);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::multMatrix (const float *m){
	multMatrix(glm::make_mat4(m));
}

//----------------------------------------------------------
void ofCustomGLRenderer::loadViewMatrix(const glm::mat4 & m){
	matrixStack.loadViewMatrix(m);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
void ofCustomGLRenderer::multViewMatrix(const glm::mat4 & m){
	matrixStack.multViewMatrix(m);
	uploadCurrentMatrix();
}

//----------------------------------------------------------
glm::mat4 ofCustomGLRenderer::getCurrentViewMatrix() const{
	return matrixStack.getViewMatrix();
}

//----------------------------------------------------------
glm::mat4 ofCustomGLRenderer::getCurrentNormalMatrix() const{
	return glm::transpose(glm::inverse(getCurrentMatrix(OF_MATRIX_MODELVIEW)));
}

//----------------------------------------------------------
void ofCustomGLRenderer::uploadCurrentMatrix(){
...
//----------------------------------------------------------
glm::mat4 ofCustomGLRenderer::getCurrentMatrix(ofMatrixMode matrixMode_) const {
	switch (matrixMode_) {
		case OF_MATRIX_MODELVIEW:
...
//----------------------------------------------------------
glm::mat4 ofCustomGLRenderer::getCurrentOrientationMatrix() const {
	return matrixStack.getOrientationMatrix();
}
//----------------------------------------------------------
void ofCustomGLRenderer::setColor(const ofColor & color){
	setColor(color.r,color.g,color.b,color.a);
}

//----------------------------------------------------------
void ofCustomGLRenderer::setColor(const ofColor & color, int _a){
	setColor(color.r,color.g,color.b,_a);
}

//----------------------------------------------------------
void ofCustomGLRenderer::setColor(int _r, int _g, int _b){
	setColor(_r, _g, _b, 255);
}

//----------------------------------------------------------
void ofCustomGLRenderer::setColor(int _r, int _g, int _b, int _a){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setColor(int gray){
	setColor(gray, gray, gray);
}

//----------------------------------------------------------
void ofCustomGLRenderer::setHexColor(int hexColor){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setBitmapTextMode(ofDrawBitmapMode mode){
	currentStyle.drawBitmapMode = mode;
}

//----------------------------------------------------------
void ofCustomGLRenderer::clear(){
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//----------------------------------------------------------
void ofCustomGLRenderer::clear(float r, float g, float b, float a) {
	glClearColor(r / 255., g / 255., b / 255., a / 255.);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
}

//----------------------------------------------------------
void ofCustomGLRenderer::clear(float brightness, float a) {
	clear(brightness, brightness, brightness, a);
}

//----------------------------------------------------------
void ofCustomGLRenderer::clearAlpha() {
	glColorMask(0, 0, 0, 1);
...
//----------------------------------------------------------
void ofCustomGLRenderer::setBackgroundAuto(bool bAuto){
	bBackgroundAuto = bAuto;
}

//----------------------------------------------------------
bool ofCustomGLRenderer::getBackgroundAuto(){
	return bBackgroundAuto;
}

//----------------------------------------------------------
ofColor ofCustomGLRenderer::getBackgroundColor(){
	return currentStyle.bgColor;
}

//----------------------------------------------------------
void ofCustomGLRenderer::setBackgroundColor(const ofColor & c){
...
//----------------------------------------------------------
void ofCustomGLRenderer::background(const ofColor & c){
	setBackgroundColor(c);
	glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
}

//----------------------------------------------------------
void ofCustomGLRenderer::background(float brightness) {
	background(ofColor(brightness));
}

//----------------------------------------------------------
void ofCustomGLRenderer::background(int hexColor, float _a){
	background ( (hexColor >> 16) & 0xff, (hexColor >> 8) & 0xff, (hexColor >> 0) & 0xff, _a);
}

//----------------------------------------------------------
void ofCustomGLRenderer::background(int r, int g, int b, int a){
	background(ofColor(r,g,b,a));
}

//----------------------------------------------------------
void ofCustomGLRenderer::setFillMode(ofFillFlag fill){
...
//----------------------------------------------------------
ofFillFlag ofCustomGLRenderer::getFillMode(){
	if(currentStyle.bFill){
		return OF_FILLED;
	}else{
		return OF_OUTLINE;
...
//----------------------------------------------------------
void ofCustomGLRenderer::setRectMode(ofRectMode mode){
	currentStyle.rectMode = mode;
}

//----------------------------------------------------------
ofRectMode ofCustomGLRenderer::getRectMode(){
	return currentStyle.rectMode;
}

//----------------------------------------------------------
void ofCustomGLRenderer::setLineWidth(float lineWidth){
...
//----------------------------------------------------------
void ofCustomGLRenderer::setDepthTest(bool depthTest) {
	if(depthTest) {
...
//----------------------------------------------------------
void ofCustomGLRenderer::setLineSmoothing(bool smooth){
	currentStyle.smoothing = smooth;
}

//----------------------------------------------------------
void ofCustomGLRenderer::startSmoothing(){
    // TODO :: needs ES2 code.
}

//----------------------------------------------------------
void ofCustomGLRenderer::endSmoothing(){
    // TODO :: needs ES2 code.
}

//----------------------------------------------------------
void ofCustomGLRenderer::setBlendMode(ofBlendMode blendMode){
	switch (blendMode){
...
//----------------------------------------------------------
void ofCustomGLRenderer::enablePointSprites(){
#ifdef TARGET_OPENGLES
...
//----------------------------------------------------------
void ofCustomGLRenderer::disablePointSprites(){
#ifdef TARGET_OPENGLES
	#ifndef TARGET_PROGRAMMABLE_GL
		glEnable(GL_POINT_SPRITE_OES);
	#endif
...
//----------------------------------------------------------
void ofCustomGLRenderer::enableAntiAliasing(){
...
//----------------------------------------------------------
void ofCustomGLRenderer::disableAntiAliasing(){
#if !defined(TARGET_PROGRAMMABLE_GL) || !defined(TARGET_OPENGLES)
...
//----------------------------------------------------------
const ofShader & ofCustomGLRenderer::getCurrentShader() const{
	return *currentShader;
}

//----------------------------------------------------------
void ofCustomGLRenderer::setAlphaBitmapText(bool bitmapText){
...
ofStyle ofCustomGLRenderer::getStyle() const{
	return currentStyle;
}

void ofCustomGLRenderer::pushStyle(){
...
void ofCustomGLRenderer::popStyle(){
...
void ofCustomGLRenderer::setStyle(const ofStyle & style){
...
void ofCustomGLRenderer::setCurveResolution(int resolution){
	currentStyle.curveResolution = resolution;
	path.setCurveResolution(resolution);
}

//----------------------------------------------------------
void ofCustomGLRenderer::setAttributes(bool vertices, bool color, bool tex, bool normals){
...
//----------------------------------------------------------
void ofCustomGLRenderer::enableTextureTarget(const ofTexture & tex, int textureLocation){
	bool wasUsingTexture = texCoordsEnabled & (currentTextureTarget!=OF_NO_TEXTURE);
...
//----------------------------------------------------------
void ofCustomGLRenderer::disableTextureTarget(int textureTarget, int textureLocation){
	bool wasUsingTexture = texCoordsEnabled & (currentTextureTarget!=OF_NO_TEXTURE);
	currentTextureTarget = OF_NO_TEXTURE;
...
//----------------------------------------------------------
GLenum ofCustomGLRenderer::getCurrentTextureTarget(){
	return currentTextureTarget;
}

//----------------------------------------------------------
void ofCustomGLRenderer::setAlphaMaskTex(const ofTexture & tex){
...
//----------------------------------------------------------
void ofCustomGLRenderer::disableAlphaMask(){
...
//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofShader & shader){
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofShader & shader){
	glUseProgram(0);
	usingCustomShader = false;
	beginDefaultShader();
}
...
//----------------------------------------------------------
void ofCustomGLRenderer::begin(const ofFbo & fbo, ofFboMode mode){
	pushView();
    pushStyle();
...
//----------------------------------------------------------
void ofCustomGLRenderer::end(const ofFbo & fbo){
	unbind(fbo);
	matrixStack.setRenderSurface(*window);
	uploadMatrices();
	popStyle();
...
//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofFbo & fbo){
...
#ifndef TARGET_OPENGLES
//----------------------------------------------------------
void ofCustomGLRenderer::bindForBlitting(const ofFbo & fboSrc, ofFbo & fboDst, int attachmentPoint){
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofFbo & fbo){
...
//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofBaseMaterial & material){
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofBaseMaterial &){
    currentMaterial = nullptr;
	beginDefaultShader();
}

//----------------------------------------------------------
void ofCustomGLRenderer::enableLighting(){

}

//----------------------------------------------------------
void ofCustomGLRenderer::disableLighting(){
}

//----------------------------------------------------------
void ofCustomGLRenderer::enableLight(int){

}

//----------------------------------------------------------
void ofCustomGLRenderer::disableLight(int){

}

//----------------------------------------------------------
bool ofCustomGLRenderer::getLightingEnabled(){
    return true;
}

//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofTexture & texture, int location){
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofTexture & texture, int location){
...
//----------------------------------------------------------
void ofCustomGLRenderer::bind(const ofCamera & camera, const ofRectangle & _viewport){
	pushView();
	viewport(_viewport);
...
//----------------------------------------------------------
void ofCustomGLRenderer::unbind(const ofCamera & camera){
	popView();
}


//----------------------------------------------------------
void ofCustomGLRenderer::uploadMatrices(){
	if(!currentShader) return;
	currentShader->setUniformMatrix4f(MODEL_MATRIX_UNIFORM, matrixStack.getModelMatrix());
	currentShader->setUniformMatrix4f(VIEW_MATRIX_UNIFORM, matrixStack.getViewMatrix());
	currentShader->setUniformMatrix4f(MODELVIEW_MATRIX_UNIFORM, matrixStack.getModelViewMatrix());
...
//----------------------------------------------------------
void ofCustomGLRenderer::setDefaultUniforms(){
	if(!currentShader) return;
...
//----------------------------------------------------------
void ofCustomGLRenderer::beginDefaultShader(){
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawLine(float x1, float y1, float z1, float x2, float y2, float z2) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawRectangle(float x, float y, float z, float w, float h) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawTriangle(float x1, float y1, float z1, float x2, float y2, float z2, float x3, float y3, float z3) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawCircle(float x, float y, float z,  float radius) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawEllipse(float x, float y, float z, float width, float height) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawString(string textString, float x, float y, float z) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
	float sx = 0;
	float sy = 0;
...
//----------------------------------------------------------
void ofCustomGLRenderer::drawString(const ofTrueTypeFont & font, string text, float x, float y) const{
	ofCustomGLRenderer * mutThis = const_cast<ofCustomGLRenderer*>(this);
...
string ofCustomGLRenderer::defaultVertexShaderHeader(GLenum textureTarget){
	return defaultShaderHeader(vertex_shader_header,textureTarget,major,minor);
}

string ofCustomGLRenderer::defaultFragmentShaderHeader(GLenum textureTarget){
	return defaultShaderHeader(fragment_shader_header,textureTarget,major,minor);
}

void ofCustomGLRenderer::setup(int _major, int _minor){
	glGetError();
#ifdef TARGET_OPENGLES
...
const ofShader * ofCustomGLRenderer::getVideoShader(const ofBaseVideoDraws & video) const{
	const ofShader * shader = nullptr;
...
void ofCustomGLRenderer::setVideoShaderUniforms(const ofBaseVideoDraws & video, const ofShader & shader) const{
	switch(video.getPixelFormat()){
		case OF_PIXELS_YUY2:
#ifndef TARGET_OPENGLES
...
int ofCustomGLRenderer::getGLVersionMajor(){
	return major;
}

int ofCustomGLRenderer::getGLVersionMinor(){
	return minor;
}


void ofCustomGLRenderer::saveFullViewport(ofPixels & pixels){
	ofRectangle v = getCurrentViewport();
	saveScreen(v.x,v.y,v.width,v.height,pixels);
}

void ofCustomGLRenderer::saveScreen(int x, int y, int w, int h, ofPixels & pixels){
	...
	if(unsigned char * p = buffer.map<unsigned char>(GL_READ_ONLY)){
		...
		ofLogError("ofCustomGLRenderer") << "Error saving screen";
	}
...
const of3dGraphics & ofCustomGLRenderer::get3dGraphics() const{
	return graphics3d;
}

of3dGraphics & ofCustomGLRenderer::get3dGraphics(){
	return graphics3d;
}
